...
## - Leave `item_id` empty to grant `amount` of `currency` (seed/money/gem/bamboo).
## - Set `item_id` to a catalog item id to grant `amount` of that item into the
##   inventory instead; `currency` is then ignored.

## Which currency this entry grants when `item_id` is empty. Matches CurrencyCatalog/HUD currency ids.
@export_enum("seed", "money", "gem", "bamboo") var currency: String = "seed"
