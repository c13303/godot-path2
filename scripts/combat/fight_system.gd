...
## otherwise the water-source refill path can advance.
...
func _update_spray_projectile_weapon(weapon: WeaponData, origin: Vector2, direction: Vector2, source_agent_id: int, delta: float, source_velocity: Vector2) -> void:
	...
	if _held_spray_weapon_id == "":
		if not ignore_water_cost and not _spend_reserve(weapon.spray_reserve_id, weapon.spray_reserve_cost):
		...
		if type_id >= 0:
			...
			if source_agent_id >= 0 and not ignore_water_cost:
				...
				if not _hose_has_shooting_fluid_at_lance():
					...
					return
			if ignore_water_cost or _spend_reserve(gun.reserve_id, gun.reserve_cost):
				var facing: Vector2 = direction.normalized()
				var spawn_pos: Vector2 = origin
				if source_agent_id >= 0:
					facing = get_current_lance_direction(direction)
					spawn_pos = get_current_lance_position(origin)
				elif gun.throw_offset != 0.0 and direction.length_squared() > 0.000001:
					spawn_pos += facing * gun.throw_offset
...
func get_current_lance_direction(fallback_direction: Vector2 = Vector2.RIGHT) -> Vector2:
	...
	if _lance != null and is_instance_valid(_lance):
		var lance_direction: Vector2 = Vector2.from_angle(_lance.global_rotation - PI * 0.5)
		if lance_direction.length_squared() > 0.000001:
			return lance_direction.normalized()
	if fallback_direction.length_squared() > 0.000001:
		return fallback_direction.normalized()
	return Vector2.RIGHT

func get_current_lance_position(fallback_position: Vector2) -> Vector2:
	if _lance == null or not is_instance_valid(_lance):
		if not is_instance_valid(_player):
			_player = get_node_or_null("../Player") as Node2D
		if _player != null:
...
func _begin_hose_shooting_flow() -> void:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid != null and _garden_hose_fluid.has_method("begin_shooting_flow"):
		_garden_hose_fluid.call("begin_shooting_flow")

func _end_hose_shooting_flow() -> void:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid != null and _garden_hose_fluid.has_method("end_shooting_flow"):
		_garden_hose_fluid.call("end_shooting_flow")

func _hose_ready_for_shooting() -> bool:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid == null:
		return false
	if _garden_hose_fluid.has_method("begin_shooting_flow"):
		_garden_hose_fluid.call("begin_shooting_flow")
...
func _begin_hose_refill_flow() -> void:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid != null and _garden_hose_fluid.has_method("begin_refill_flow"):
		_garden_hose_fluid.call("begin_refill_flow")

func _end_hose_refill_flow() -> void:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid != null and _garden_hose_fluid.has_method("end_refill_flow"):
		_garden_hose_fluid.call("end_refill_flow")

func _hose_ready_for_refill() -> bool:
	if _garden_hose_fluid == null or not is_instance_valid(_garden_hose_fluid):
		_garden_hose_fluid = get_node_or_null("../visualFX/GardenHose/GardenHoseFluid")
	if _garden_hose_fluid == null:
		return false
	if not _garden_hose_fluid.has_method("is_ready_for_refill"):
		return true
	return bool(_garden_hose_fluid.call("is_ready_for_refill"))
