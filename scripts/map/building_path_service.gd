...
# the source-of-truth state (topology, pathfinder, telemetry) and thin compatibility
# wrappers; this service reaches back through _manager for all of it.
...
func find_path_on_walkable_map(from_tile: Vector2i, to_tile: Vector2i) -> PackedVector2Array:
	var pf: Node = _manager.get("pathfinder") as Node
	if pf == null or not pf.has_method("find_path"):
		return PackedVector2Array()
	var topo: Variant = _manager.get("_garden_topology")
...
func find_path_in_zone(from_tile: Vector2i, to_tile: Vector2i, garden_id: int = 0) -> PackedVector2Array:
	...
	var telemetry: BuildingDebugTelemetry = _manager.get("_debug_telemetry") as BuildingDebugTelemetry
	...
	var end_tile: Vector2i = to_tile if path_tiles.has(to_tile) else _nearest_zone_tile_to(to_tile, path_tiles)
	if start_tile == INVALID_CELL or end_tile == INVALID_CELL:
		_manager.call("_accumulate_find_path_in_zone", call_start_us, sync_elapsed, 0, from_tile, to_tile, path_tiles.size())
		return PackedVector2Array()
	...
	_manager.call("_accumulate_find_path_in_zone", call_start_us, sync_elapsed, find_elapsed, from_tile, to_tile, path_tiles.size())
	return result

func sync_pathfinder_zone_tiles(zone_tiles: Dictionary) -> void:
	_manager.set("_last_zone_blocker_us", 0)
	var pf: Node = _manager.get("pathfinder") as Node
	if pf == null:
		return
	...
	if pf.has_method("set_blockers"):
		...
		_manager.set("_last_zone_blocker_us", blocker_elapsed)
...
func wall_blockers_for_cells(cells: Dictionary) -> PackedVector2Array:
	...
	var wallz_layer: TileMapLayer = _manager.get("wallz") as TileMapLayer
	var blocking_buildings_layer: TileMapLayer = _manager.get("blocking_buildings") as TileMapLayer
...
func _building_cell_blocks_movement(cell: Vector2i) -> bool:
	return bool(_manager.call("_building_cell_blocks_movement", cell))
